...
namespace Madhuvan.Datalayer
{
    public class GenericRepository<T> where T : class
    {
        ...
        {
            MadhuvanMVCEntities context = BaseContext.GetDbContext();
        ...
        {
            using (MadhuvanMVCEntities context = BaseContext.GetDbContext())
            {
        ...
        public string Insert(T entity)
        {
            ...
            catch (DbEntityValidationException dbEx)
            {
                ...
                return CommonHelper.GetErrorMessage(dbEx);
            }
        }
        ...
        public void Insert(T entity, MadhuvanMVCEntities context)
        {
            DbSet<T> table = context.Set<T>();
            table.Add(entity);
        }
        ...
        /// <param name="entity">entity to be updated</param>
        /// <param name="context">context to maintain transaction</param>
        public void Update(T entity, MadhuvanMVCEntities context)
        {
            DbSet<T> table = context.Set<T>();
            table.Attach(entity);
            context.Entry(entity).State = EntityState.Modified;
        ...
        /// <param name="pk">Primary key of entity to be deleted</param>
        /// <param name="context">context to maintain transaction</param>
        public void Delete(object pk, MadhuvanMVCEntities context)
        {
            DbSet<T> table = context.Set<T>();
            T existing = table.Find(pk);
            context.Entry(existing).State = EntityState.Deleted;
        ...
        public List<T> Search(Expression<Func<T, bool>> filter = null)
        {
            using (MadhuvanMVCEntities context = BaseContext.GetDbContext())
            {
                return filter == null ? context.Set<T>().ToList<T>() : context.Set<T>().Where<T>(filter).ToList<T>();
            }
        }
